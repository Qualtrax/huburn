    ...
    function cycleTimeService(dateService, issueService) {
        ...
        function getStandardDeviationSummary(currentMilestoneIssues, historicalIssues) {
            ...
            for (var i = 0; i < pointsLabelsToRetrieve.length; i++) {
                ...
                var inProgressTimes = getCycleTimesForColumn(issuesWithLabel, '2 - In Progress').filter(function (timeObject) {
                    return timeObject.time > 0;
                });

                if (inProgressTimes.length < 2)
                    continue;

                var inProgressTimesInDays = inProgressTimes.map(function (timeObject) {
                    timeObject.time = dateService.toDays(timeObject.time);
                    return timeObject;
                });

                var inProgressTimesOnly = inProgressTimesInDays.map(function (timeObject) { return timeObject.time; });
                var mean = jStat.mean(inProgressTimesOnly);
                var standardDeviation = jStat.stdev(inProgressTimesOnly);
                var minValue = mean - (3 * standardDeviation);
                ...
                var currentInProgressTimes = getCycleTimesForColumn(currentIssuesWithLabel, '2 - In Progress').filter(function (timeObject) {
                    return timeObject.time > 0;
                });

                var currentInProgressTimesInDays = currentInProgressTimes.map(function (timeObject) {
                    timeObject.time = dateService.toDays(timeObject.time);
                    return timeObject;
                });

                if (currentInProgressTimesInDays.length > 0) {
                    statistics.push({
                        pointsLabel: pointsLabelsToRetrieve[i],
                        numberOfHistoricalItems: inProgressTimesOnly.length,
                        dataPoints: currentInProgressTimesInDays,
                        mean: mean,
            ...
            var collectiveMinValue = 0;
            
            if (minValues.length > 0)
                collectiveMinValue = jStat.min(minValues);

            var collectiveMaxValue = 0;

            if (maxValues.length > 0)
                collectiveMaxValue = jStat.max(maxValues);

            return  {
                statistics,
                collectiveMinValue: collectiveMinValue,
                collectiveMaxValue: collectiveMaxValue,
                collectiveRange: collectiveMaxValue - collectiveMinValue
            };
        }
