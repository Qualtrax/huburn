(function() {
  ...
  angular.module('huburn').directive('huburnBurndownGraph', [ '$window', 'd3', function($window, d3) {
    return {
      restrict: 'E',
      scope: {
        data: '='
      ...
      link: function(scope, element, attrs) {
        ...
        var render = function(data) {
          ...
          var width = rect.right - rect.left - 150;
          ...
          svg.append('g')
            ...
            .enter()
              .append('circle')
              .attr({
                cx: function(d,i) { return pastDays.length > 0 ? padding * 2.5 + xScale(xValue(pastDays[pastDays.length-1])) : padding * 2.5; },
                cy: function(d) { return padding + yScale(yValue(d)); },
                r: 5
              })
              .on('mouseover', tip.show)
          ...
          var getPointsRemainingClass = function(data, pastDays) {
            var idealPointsRemaining = pastDays.length > 0 ? data.idealLine[pastDays.length-1].remaining : 0;
            var halfWayPoint = data.remaining - data.pointsInProgress / 2;
            var untouched = data.remaining - data.pointsInProgress

            if (data.remaining <= idealPointsRemaining) return 'super-green';
            else if (halfWayPoint <= idealPointsRemaining) return 'green';
            else if (untouched <= idealPointsRemaining) return 'yellow';
            else return 'red';
          };

          var pointsRemainingClass = getPointsRemainingClass(data, pastDays);
          pointsRemaining.append('text').attr({ class: "value " + pointsRemainingClass, x: 0, y: 28 + 28 + 48 }).text(data.remaining);
        };
