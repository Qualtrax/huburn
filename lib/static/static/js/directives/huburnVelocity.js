      ...
      return {
        ...
        link: function(scope, element, attrs) {
          ...
          function renderBadges(data, x, y) {
            ...
            var numberOfFirelanes = (velocityMilestones.map(firelanes).reduce(sum) / milestonesInVelocity).toFixed(1);
            appendBadge(badges, 390, 0, "Firelanes", numberOfFirelanes, firelaneClass);

            var numberOfEscalations = (velocityMilestones.map(escalations).reduce(sum) / milestonesInVelocity).toFixed(1);
            appendBadge(badges, 530, 0, "Escalations", numberOfEscalations, escalationClass);

            var numberOfScopeChanges = (velocityMilestones.map(scopeChanges).reduce(sum) / milestonesInVelocity).toFixed(1);
            appendBadge(badges, 700, 0, "Scope Changes", numberOfScopeChanges, noValueClass);
          };
