(function () {
    ...
    function metricsCtrl($routeParams, $q, metricService, gitHubService, cycleTimeService, dateService) {
        ...
        function activate() {
            ...
            getCurrentMilestone()
                ...
                .then(function (issuesWithEventsForCurrentMilestone) {
                    ...
                    try {
                        cachedIssues = JSON.parse(localStorage.getItem('historicalIssues'));
                    }
                    catch (e) { }

                    if (cachedIssues) {
                        loadMetrics(cachedIssues)
                        finishLoading();
                    }
                    else {
                        getMilestones()
        ...
        function loadMetrics(issuesWithEvents) {
            localStorage.setItem('historicalIssues', JSON.stringify(issuesWithEvents));
